...
@Component({
  ...
  template: `
  <aside class="h-full flex flex-col bg-theme-elev border-r border-theme">
  <div class="p-4 border-b border-theme flex items-center justify-between">
    ...
    <div class="p-3">
  <p class="text-[11px] uppercase tracking-wide text-theme-muted mb-1">Recent Sessions</p>
      <div class="space-y-1 max-h-64 overflow-auto pr-1">
        <button *ngFor="let s of sessions()" (click)="selectSession.emit(s.id)"
          class="w-full text-left px-3 py-2 rounded-md text-sm hover:bg-theme-secondary transition flex flex-col gap-0.5"
          [style.background]="s.id === activeSessionId ? 'var(--bg-elev)' : ''">
          <span class="font-medium line-clamp-1">{{ s.title || 'Untitled' }}</span>
          <span class="text-[10px] text-theme-muted">{{ formatTs(s.updatedAt) }}</span>
        </button>
        <p *ngIf="sessions().length === 0" class="text-xs text-slate-500">No sessions yet</p>
      </div>
    </div>

  <div class="mt-auto p-3 border-t border-theme">
  <p class="text-[11px] uppercase tracking-wide text-theme-muted mb-1">Documents</p>
